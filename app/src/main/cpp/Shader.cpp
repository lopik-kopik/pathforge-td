...
Shader *Shader::loadShader(
    ...
    if (program) {
        ...
        if (linkStatus != GL_TRUE) {
            ...
            GLint modelUniform = glGetUniformLocation(program, "uModel");
            GLint colorUniform = glGetUniformLocation(program, "uColor");
            GLint uvTransformUniform = glGetUniformLocation(program, "uUVTransform");

            // Only create a new shader if all the required attributes are found.
            if (positionAttribute != -1
                ...
                shader = new Shader(
                        ...
                        modelUniform,
                        colorUniform);
                shader->uvTransform_ = uvTransformUniform;
            } else {
                glDeleteProgram(program);
            }
        }
...
void Shader::setModelMatrix(float *modelMatrix) const {
    if (model_ != -1) {
        glUniformMatrix4fv(model_, 1, false, modelMatrix);
    }
}
...
void Shader::setUVTransform(float offsetU, float offsetV, float scaleU, float scaleV) const {
    if (uvTransform_ != -1) {
        glUniform4f(uvTransform_, offsetU, offsetV, scaleU, scaleV);
    }
}